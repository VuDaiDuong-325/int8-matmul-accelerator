...
// [QUAN TRỌNG] Tăng MAP_SIZE lên 16KB để đủ chứa mảng 16x512 = 8192 bytes
#define MAP_SIZE 16384UL  
#define MAP_MASK (MAP_SIZE - 1)

// Scale up ma trận lên 512x512
#define M_TOTAL 512
#define K_TOTAL 512
#define N_TOTAL 512
#define M_TILE 16
...
int main() {
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            *((volatile uint32_t *)(gpio_k_base + 0x00)) = K_TOTAL;
            ...
            int wait_timeout = 10000000;
            while ((dma_get(dma0_base, MM2S_SR) & 0x02) == 0 && wait_timeout > 0) wait_timeout--;

            int timeout = 10000000; 
            while ((dma_get(dma1_base, S2MM_SR) & 0x02) == 0 && timeout > 0) timeout--;
            
            if (timeout == 0 || wait_timeout == 0) { 
    ...
    // Tên file lưu ra đã được đổi thành 512x512
