...
int main() {
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            *((volatile uint32_t *)(gpio_k_base + 0x00)) = K_TOTAL;
            ...
            // Kích thước mảng A, B = 16384 bytes. Chia 2 nửa, mỗi nửa 8192 bytes.
            // ================================================================
            size_t half_bytes = bytes_a / 2; 
            ...
            // Đợi DMA đẩy xong nhịp 1 (Mỗi kênh MM2S phải chờ cờ Idle bật lên)
            int wait1 = 10000000;
            ...
            // Lưu ý: Cộng thêm half_bytes vào địa chỉ RAM để truyền tiếp phần sau
    ...
    // Tên file lưu ra đã được đổi thành 1024x1024
