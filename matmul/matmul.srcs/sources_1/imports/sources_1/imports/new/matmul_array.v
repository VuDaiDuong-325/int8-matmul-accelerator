module matmul_array #(
    parameter ARRAY_SIZE = 4  //pe 16x16
)(
    input  wire clk,
    input  wire rst_n,
    ...
    input  wire [(ARRAY_SIZE*32)-1:0]          psum_in_left,
    ...
    wire [31:0] psum_wire   [0:ARRAY_SIZE-1][0:ARRAY_SIZE];
    ...
    generate
        for (i = 0; i < ARRAY_SIZE; i = i + 1) begin : BIND_INPUTS
            ...
            assign psum_wire[i][0] = psum_in_left[(i*32)+31 : i*32];
        end
    endgenerate
