...
// [QUAN TRỌNG] Tăng MAP_SIZE lên 64KB để thoải mái chứa mảng 16x1024 = 16KB
#define MAP_SIZE 65536UL  
#define MAP_MASK (MAP_SIZE - 1)

// Scale up ma trận lên 1024x1024
#define M_TOTAL 1024
#define K_TOTAL 1024
#define N_TOTAL 1024
#define M_TILE 16
...
int main() {
    int fd;
    // Sử dụng static để lưu ở phân vùng BSS, tránh tràn Stack (Stack Overflow)
    ...
    srand(time(NULL));
    printf("[1/3] Generating random INT8 data (1024x1024)... ");
    ...
    size_t bytes_a = M_TILE * K_TOTAL * sizeof(int8_t); // 16384 Bytes
    size_t bytes_b = K_TOTAL * N_TILE * sizeof(int8_t); // 16384 Bytes

    printf("[2/3] Running FPGA Accelerator Ping-Pong (K_TOTAL = %d)...\n", K_TOTAL);
    
    gettimeofday(&sys_start, NULL);
    
    // Set K_DIM cho NPU qua GPIO
    *((volatile uint32_t *)(gpio_k_base + 0x00)) = K_TOTAL;
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            // 1. CHUẨN BỊ KÊNH NHẬN (C: 1024 bytes -> Rất an toàn, < 16383)
            ...
            // ================================================================
            // 2. CHIA NHỎ KÊNH GỬI (A, B) ĐỂ LÁCH LỖI TRÀN 14-BIT CỦA DMA
            // ================================================================
            size_t half_bytes = bytes_a / 2; // = 8192 Bytes

            // --- Nhịp 1: Gửi 8192 bytes đầu tiên ---
            dma_set(dma1_base, MM2S_SA, MEM_TX_A_ADDR);
            dma_set(dma0_base, MM2S_SA, MEM_TX_B_ADDR);
            dma_set(dma1_base, MM2S_LENGTH, half_bytes);
            dma_set(dma0_base, MM2S_LENGTH, half_bytes);

            // Đợi DMA đẩy xong nhịp 1
            int wait1 = 10000000;
            while ((dma_get(dma1_base, MM2S_SR) & 0x02) == 0 && wait1 > 0) wait1--;
            int wait2 = 10000000;
            while ((dma_get(dma0_base, MM2S_SR) & 0x02) == 0 && wait2 > 0) wait2--;

            // --- Nhịp 2: Gửi 8192 bytes còn lại ---
            dma_set(dma1_base, MM2S_SA, MEM_TX_A_ADDR + half_bytes);
            dma_set(dma0_base, MM2S_SA, MEM_TX_B_ADDR + half_bytes);
            dma_set(dma1_base, MM2S_LENGTH, half_bytes);
            dma_set(dma0_base, MM2S_LENGTH, half_bytes);
            
            // Đợi DMA đẩy xong nhịp 2
            int wait3 = 10000000;
            while ((dma_get(dma1_base, MM2S_SR) & 0x02) == 0 && wait3 > 0) wait3--;
            int wait4 = 10000000;
            while ((dma_get(dma0_base, MM2S_SR) & 0x02) == 0 && wait4 > 0) wait4--;

            // ================================================================
            // 3. CHỜ NPU NHẢ KẾT QUẢ VÀO KÊNH NHẬN (C)
            // ================================================================
            int timeout = 10000000; 
            while ((dma_get(dma1_base, S2MM_SR) & 0x02) == 0 && timeout > 0) timeout--;
            
            if (timeout == 0 || wait1 == 0 || wait2 == 0 || wait3 == 0 || wait4 == 0) { 
            ...
            for (int i = 0; i < M_TILE; i++) {
                for (int j = 0; j < N_TILE; j++) {
                    // Logic lật ngược hàng đặc trưng của kiến trúc NPU
    ...
    dump_int8_to_file("input_A1024x1024.txt", (int8_t *)master_A, M_TOTAL, K_TOTAL);
    dump_int8_to_file("input_B1024x1024.txt", (int8_t *)master_B, K_TOTAL, N_TOTAL);
    dump_int32_to_file("output_C1024x1024_FPGA.txt", (int32_t *)hw_C, M_TOTAL, N_TOTAL);
    printf("Done.\n");
