...
int main() {
    ...
    printf("[2/3] Running FPGA with Ping-Pong K-TILING (1024 + 256)...\n");
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        ...
        for (int c = 0; c < num_chunks; c++) {
            ...
            // TỐI ƯU CỦA BẠN: Bơm Tile A ở vòng lặp ngoài
            int idx_a = 0;
            ...
            for (int n = 0; n < N_TOTAL; n += N_TILE) {
                ...
                *((volatile uint32_t *)(gpio_k_base + 0x00)) = k_len;
                ...
                // 1. Luôn kích hoạt kênh nhận (S2MM) trước để hứng data an toàn
                ...
                if (k_len == 1024) {
                    // DMA Limit hack cho khúc 1024 (Chia đôi luồng 16KB)
                ...
                // Chờ NPU Ping-Pong xuất xong kết quả C
                ...
                // Reset K_DIM để đảm bảo chu kỳ sau an toàn
                *((volatile uint32_t *)(gpio_k_base + 0x00)) = 0;
                
                // ========================================================
                // CỘNG DỒN KẾT QUẢ & LẬT NGƯỢC (Ping-Pong Architecture)
                // Phải dùng += vì ghép 2 mảnh K (1024 và 256) lại với nhau
                // ========================================================
                int idx_c = 0;
                for (int i = 0; i < M_TILE; i++) {
                    for (int j = 0; j < N_TILE; j++) {
                        // Đảo chiều (M_TILE - 1 - i) cho đúng kiểu drain từ đáy lên
                        hw_C[m + (M_TILE - 1 - i)][n + j] += rx_c_ram[idx_c++];
                    }
                }
            }
            k_offset += k_len; // Dịch offset để chuẩn bị chạy khúc K=256
        }
    }
