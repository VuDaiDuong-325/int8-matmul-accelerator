...
int main() {
    ...
    // Cấp phát động khoảng 9.6 MB RAM
    ...
    if (!master_A || !master_B || !hw_C) {
        printf("[ERR] Lỗi cấp phát bộ nhớ RAM! (Thiếu RAM)\n");
        return -1;
    }
    ...
    *((volatile uint32_t *)(gpio_k_base + 0x00)) = (uint32_t)K_HW;
    ...
    printf("[INFO] Running FPGA Accelerator (%dx%d) - Total 512,000 HW blocks...\n", M_TOTAL, N_TOTAL);
    ...
    printf("[INFO] Exporting TXT files (Please wait, this will take a while for ~40MB of text)...\n");
