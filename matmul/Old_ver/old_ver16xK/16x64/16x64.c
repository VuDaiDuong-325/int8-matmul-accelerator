...
// Kích thước ma trận tổng quát (A: 16x64, B: 64x16) -> C: 16x16
#define M_TOTAL 16
#define K_TOTAL 64
#define N_TOTAL 16

// Kích thước Tiling cho phần cứng NPU 16x16
#define M_TILE 16
#define N_TILE 16
#define K_HW   16  
...
void dump_int8_to_file(const char* filename, int8_t *mat, int rows, int cols) {
    ...
    if (!f) { printf("[ERR] Cannot create %s\n", filename); return; }
...
int main() {
    ...
    printf("[INFO] NPU Mode: Tiling %dx%d_k%d | Total: %dx%d * %dx%d\n", 
            M_TILE, N_TILE, K_HW, M_TOTAL, K_TOTAL, K_TOTAL, N_TOTAL);
    ...
    *((volatile uint32_t *)(gpio_k_base + 0x00)) = (uint32_t)K_HW;
    ...
    printf("[2/3] Running FPGA Accelerator... ");
    fflush(stdout);
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            for (int k = 0; k < K_TOTAL; k += K_HW) {
                ...
                uint32_t actual_bytes = dma_get(dma1_base, S2MM_LENGTH);
                printf("\n[DEBUG] DMA da nhan duoc: %d bytes (Ky vong: %d bytes)\n", actual_bytes, bytes_c);
                
                // Cộng dồn kết quả các block K
                for (int i = 0; i < M_TILE; i++) {
                    for (int j = 0; j < N_TILE; j++) partial_sum[i][j] += rx_c_ram[i * N_TILE + j];
                }
            }
            // Gán giá trị hoàn chỉnh vào ma trận C
            for (int i = 0; i < M_TILE; i++) {
    ...
    fflush(stdout);
    dump_int8_to_file("input_A16x64.txt", (int8_t *)master_A, M_TOTAL, K_TOTAL);
    dump_int8_to_file("input_B64x16.txt", (int8_t *)master_B, K_TOTAL, N_TOTAL);
    dump_int32_to_file("output_C16x16_FPGA.txt", (int32_t *)hw_C, M_TOTAL, N_TOTAL);
    printf("Done.\n");
