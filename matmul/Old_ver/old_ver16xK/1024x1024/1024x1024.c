...
int main() {
    ...
    *((volatile uint32_t *)(gpio_k_base + 0x00)) = (uint32_t)K_HW;
    ...
    printf("[INFO] Running FPGA Accelerator (%dx%d) - Total 262,144 HW blocks...\n", M_TOTAL, N_TOTAL);
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            for (int k = 0; k < K_TOTAL; k += K_HW) {
                ...
                if (timeout == 0) { 
                    printf("[ERR] DMA Timeout at m=%d, n=%d, k=%d!\n", m, n, k); 
                    return -1; 
                }
    ...
    printf("[INFO] Exporting TXT files (This will take a few seconds)...\n");
    ...
    printf("[INFO] All finished successfully!\n");
    return 0;
}