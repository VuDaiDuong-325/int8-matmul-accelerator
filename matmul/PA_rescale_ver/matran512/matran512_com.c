...
int main(void) {
    ...
    static int8_t cpu_C[M_TOTAL][N_TOTAL];  // Ma trận kết quả của CPU
    static int8_t hw_C[M_TOTAL][N_TOTAL];   // Ma trận kết quả của NPU

    struct timeval sys_start, sys_end, hw_start, hw_end;
    struct timeval cpu_start, cpu_end; // Biến đo thời gian cho CPU
    double hw_pure_us = 0.0, sys_total_ms = 0.0, cpu_total_ms = 0.0;

    srand(time(NULL));
    printf("[1/4] Generating random INT8 data (%dx%d)... ", M_TOTAL, N_TOTAL);
    fflush(stdout);
    ...
    // ──────────────────────────────────────────────────────────────
    // MỚI: CHẠY MA TRẬN TRÊN CPU VÀ ĐO THỜI GIAN (CÓ TIẾN ĐỘ)
    // ──────────────────────────────────────────────────────────────
    printf("[2/4] Running CPU Baseline Matrix Multiplication...\n");
    gettimeofday(&cpu_start, NULL);

    for (int i = 0; i < M_TOTAL; i++) {
        if (i % 4 == 0 || i == M_TOTAL - 1) {
        ...
        for (int j = 0; j < N_TOTAL; j++) {
            ...
            int64_t w_ext = (int64_t)acc; // Sign-extend 32 -> 34 bit (C dùng 64-bit an toàn)
            int64_t round_bias = 0;
            if (SCALE_SHIFT > 0) {
                round_bias = (int64_t)1 << (SCALE_SHIFT - 1); // round_bias = 1 << (scale_shift - 1)
            }
            int64_t w_rnd = w_ext + round_bias;
            int64_t w_shr = w_rnd >> SCALE_SHIFT; // Phép dịch phải số có dấu (Arithmetic right shift)
            int64_t w_zp  = w_shr + (int64_t)(int8_t)ZERO_POINT; // Cộng zero_point có dấu
    ...
    // ──────────────────────────────────────────────────────────────
    // GIỮ NGUYÊN: PHẦN CHẠY TRÊN NPU PHẦN CỨNG
    // ──────────────────────────────────────────────────────────────
    ...
    // --- KIỂM TRA ĐỘ CHÍNH XÁC (Xác thực chéo CPU và NPU) ---
    int errors = 0;
    for (int i = 0; i < M_TOTAL; i++) {
        for (int j = 0; j < N_TOTAL; j++) {
    ...
    // ──────────────────────────────────────────────────────────────
    // MỚI: IN KẾT QUẢ SO SÁNH HIỆU NĂNG RÕ RÀNG
    // ──────────────────────────────────────────────────────────────
    printf("\n==================================================\n");
    printf(">> PERFORMANCE & ACCURACY COMPARISON REPORT <<\n");
    printf("==================================================\n");
    printf("Matrix Size               : %d x %d x %d (INT8)\n", M_TOTAL, K_TOTAL, N_TOTAL);
    printf("1. CPU Execution Time     : %.3f ms\n", cpu_total_ms);
    printf("2. Pure NPU Hardware Time : %.3f ms\n", hw_pure_us / 1000.0);
    printf("3. Total NPU System Time  : %.3f ms (Includes Linux overhead/driver)\n", sys_total_ms);
    printf("--------------------------------------------------\n");
    ...
    dump_int8("output_C512x512_CPU.txt", (int8_t *)cpu_C,    M_TOTAL, N_TOTAL);
