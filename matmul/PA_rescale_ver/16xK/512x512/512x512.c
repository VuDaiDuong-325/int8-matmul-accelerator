...
#include <stdlib.h>
#include <stdint.h>
#include <string.h>    // Thêm thư viện string.h để dùng memcpy
#include <unistd.h>
#include <fcntl.h>
#include <sys/mman.h>
#include <time.h>
...
int main(void) {
    ...
    // STAGING BUFFERS: Bộ đệm trung gian lưu trong Cache (Chạy nhanh)
    ...
    for (int m = 0; m < M_TOTAL; m += M_TILE) {
        for (int n = 0; n < N_TOTAL; n += N_TILE) {
            ...
            for (int t = 0; t < NUM_K_TILES; t++) {
                ...
                // --- 1. CPU GÓI DỮ LIỆU VÀO CACHE TRƯỚC (Rất Nhanh) ---
                int idx = 0;
                ...
                idx = 0;
                for (int k = k_off; k < k_off + K_DIM; k++) {
                    memcpy(&buf_b[idx], &master_B[k][n], N_TILE); // Tối ưu B
                    idx += N_TILE;
                }

                // --- 2. BURST WRITE XUỐNG DMA (Tốc độ ánh sáng) ---
                memcpy(tx_a, buf_a, bytes_a);
                memcpy(tx_b, buf_b, bytes_b);

                // --- 3. ĐO PURE HARDWARE TIME (Chỉ tính DMA và NPU) ---
                ...
                int to = 10000000;
                while (!(reg_read(dma1, MM2S_SR) & 0x1000) && to > 0) to--;
                while (!(reg_read(dma0, MM2S_SR) & 0x1000) && to > 0) to--;
                
                if (to == 0) { printf("\n[ERR] MM2S TIMEOUT\n"); return -1; }

                gettimeofday(&hw_end, NULL);
                hw_pure_us += (hw_end.tv_sec - hw_start.tv_sec) * 1e6 + (hw_end.tv_usec - hw_start.tv_usec);
            }

            // Chờ S2MM NPU tính xong và trả C
            gettimeofday(&hw_start, NULL);
            int to = 10000000;
            while (!(reg_read(dma1, S2MM_SR) & 0x1000) && to > 0) to--;
            gettimeofday(&hw_end, NULL);
            hw_pure_us += (hw_end.tv_sec - hw_start.tv_sec) * 1e6 + (hw_end.tv_usec - hw_start.tv_usec);

            // --- 4. BURST READ LẤY KẾT QUẢ TỪ DMA LÊN CACHE ---
            memcpy(buf_c, rx_c, bytes_c);

            // --- 5. CPU GIẢI NÉN KẾT QUẢ TỪ CACHE (Rất Nhanh) ---
            int idx_c = 0;
            for (int i = 0; i < M_TILE; i++) {
