...
int main(void) {
    ...
    static int8_t cpu_C[M_TOTAL][N_TOTAL];  // Dùng đối chiếu nội bộ
    static int8_t hw_C[M_TOTAL][N_TOTAL];   // Ma trận kết quả thực tế từ NPU
    ...
    // Đọc file input_A_512x512.bin
    FILE *f_a = fopen("input_A_512x512.bin", "rb");
    if (!f_a) {
        perror("\n[ERR] Cannot open input_A_512x512.bin");
        close(fd); return -1;
    }
    size_t read_a = fread(master_A, sizeof(int8_t), M_TOTAL * K_TOTAL, f_a);
    fclose(f_a);
    if (read_a != M_TOTAL * K_TOTAL) {
        printf("\n[ERR] input_A_512x512.bin bị lỗi kích thước hoặc thiếu dữ liệu!\n");
        close(fd); return -1;
    }

    // Đọc file input_B_512x512.bin
    FILE *f_b = fopen("input_B_512x512.bin", "rb");
    if (!f_b) {
        perror("\n[ERR] Cannot open input_B_512x512.bin");
        close(fd); return -1;
    }
    size_t read_b = fread(master_B, sizeof(int8_t), K_TOTAL * N_TOTAL, f_b);
    fclose(f_b);
    if (read_b != K_TOTAL * N_TOTAL) {
        printf("\n[ERR] input_B_512x512.bin bị lỗi kích thước hoặc thiếu dữ liệu!\n");
        close(fd); return -1;
    }
    ...
    // ──────────────────────────────────────────────────────────────
    printf("[4/4] Exporting hardware result to output_C512x512_NPU.bin... ");
    fflush(stdout);

    FILE *f_c_npu = fopen("output_C512x512_NPU.bin", "wb");
    if (f_c_npu) {
        fwrite(hw_C, sizeof(int8_t), M_TOTAL * N_TOTAL, f_c_npu);
        fclose(f_c_npu);
        printf("Done.\n");
    } else {
        printf("\n[ERR] Ghi file kết quả output_C512x512_NPU.bin thất bại!\n");
    }
